...
import com.app.entity.admin.Role;
import com.app.entity.admin.User;
import com.app.service.admin.AuthorityService;
import com.app.service.admin.LogService;
import com.app.service.admin.MenuService;
import com.app.service.admin.RoleService;
import com.app.service.admin.UserService;
import com.app.util.CpachaUtil;
...
public class SystemController {
	...
	public ModelAndView index2(ModelAndView model,HttpServletRequest request){
		Map<String, String> ret = new HashMap<String, String>();
		
		User findByUsername = userService.findByUsername("sa");
		if(findByUsername == null){
			ret.put("type", "error");
			ret.put("msg", "该用户名不存在！");
			logService.add("登录时，用户名为sa的用户不存在!");
		}
		if(!"123456".equals(findByUsername.getPassword())){
			ret.put("type", "error");
			ret.put("msg", "密码错误！");
			logService.add("用户名为sa的用户登录时输入密码错误!");
		}
		...
		ret.put("type", "success");
		ret.put("msg", "登录成功！");
		logService.add("用户名为{sa}，角色为{普通用户}的用户登录成功!");
